...
function initMap() {
	var map = new google.maps.Map(document.getElementById('map'), {
		zoom: 16,
		center: {lat:37.5038326,lng:127.0472178} // 회사 위치
	});
	...
	$.get('https://spreadsheets.google.com/feeds/list/1ow0jxnIgGtiuFnxdhWgZsh_4v6QJqQLbABcg_zzmaLg/1/public/values?alt=json', function(data){
		...
		for(var i = 0; i < entries.length; i++){
			...
			place.name = entry['gsx$밥집이름'] ? entry['gsx$밥집이름']['$t'] : '';
			...
			place.writer = entry['gsx$emailaddress'] ? entry['gsx$emailaddress']['$t'] : '';
			place.created = entry['gsx$timestamp'] ? entry['gsx$timestamp']['$t'] : '';
			...
			.on('click', function(e){
				...
				DISQUS.reset({
					...
					config: function () {  
						...
						this.page.title = place.name;  
						this.page.url = 'https://younghoo-codestates.github.io/coshelin-guide#!' + place.id;
					}
				});
			});
